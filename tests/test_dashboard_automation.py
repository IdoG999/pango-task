...
@pytest.mark.bug_02
@pytest.mark.tc_07
def test_slot_input_validation_expected_behavior(authenticated_dashboard):
    dashboard = authenticated_dashboard
    dashboard.create_or_attempt_parking(
        plate=test_data.VALID_PLATE_A,
        slot=test_data.INVALID_SLOT_VALUE,
    )
    # Expected behavior from test-plan: invalid slot input should be rejected.
    assert dashboard.has_slot_validation_error()
...
@pytest.mark.bug_03
@pytest.mark.tc_06
def test_duplicate_slot_assignment_expected_behavior(authenticated_dashboard):
    dashboard = authenticated_dashboard
    target_slot = dashboard.first_available_slot()
    dashboard.create_or_attempt_parking(plate=test_data.VALID_PLATE_A, slot=target_slot)
    assert dashboard.is_plate_visible_in_active_table(test_data.VALID_PLATE_A)
    ...
    dashboard.create_or_attempt_parking(plate=test_data.VALID_PLATE_B, slot=target_slot)
    # Expected behavior from test-plan: second vehicle should be blocked.
    assert dashboard.has_duplicate_slot_blocking_error()
    assert not dashboard.is_plate_visible_in_active_table(test_data.VALID_PLATE_B)


@pytest.mark.high
@pytest.mark.bug_01
@pytest.mark.tc_09
def test_vehicle_type_multiple_choices_expected_behavior(authenticated_dashboard):
    dashboard = authenticated_dashboard
    # Expected behavior from test-plan: more than one vehicle type should be available.
    assert dashboard.vehicle_type_option_count() > 1
